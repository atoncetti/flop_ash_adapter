defmodule FlopAshAdapter.Generators do
  @moduledoc false
  ...
  def pet do
    gen all(
          ...
          age <- integer(1..500),
          species <- string(:alphanumeric, min_length: 2)
        ) do
  ...
  def filterable_pet_field do
    member_of(Flop.Schema.filterable(%MyApp.Thing.Pet{}))
  end
  ...
  def uniq_list_of_owners(len) do
    gen all(
          names <- uniq_list_of_strings(len),
          ages <- uniq_list_of(integer(1..500), length: len),
          emails <- uniq_list_of_strings(len)
        ) do
      [names, ages, emails]
      |> Enum.zip()
      |> Enum.map(fn {name, age, email} ->
        %MyApp.Schema.Owner{name: name, age: age, email: email}
      end)
    end
  end
  ...
  def pagination_parameters(type) when type in [:first, :last] do
    gen all(
          val_1 <- positive_integer(),
          val_2 <- one_of([string(:alphanumeric), constant(nil)])
        ) do
      case type do
  ...
  def filter do
    gen all(
          field <- member_of([:age, :name, :owner_name]),
          value <- value_by_field(field),
          op <- operator_by_type(value)
        ) do
  ...
  defp search_text(s) when is_binary(s) do
    gen all(
          whitespace_character <- member_of(@whitespace),
          text_list <- search_text_list(s)
        ) do
